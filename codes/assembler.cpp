...
std::string assembler::TranslateOprand(int current_address, std::string str, int opcode_length) {
	...
	if (!(item.getType() == vAddress && item.getVal() == -1)) {
		...
		auto address_diff = label_address - current_address - 1;
		auto bin_diff = NumberToAssemble(address_diff);
		bin_diff = bin_diff.substr(16 - opcode_length, opcode_length);
		return bin_diff;
	}
	if (str[0] == 'R') {
		// str is a register
		...
		auto bin = NumberToAssemble(num);
		bin = bin.substr(16 - opcode_length, opcode_length);
		return bin;
	}
}
...
int assembler::assemble(std::string input_filename, std::string output_filename) {
	...
	for (int line_index = 0; line_index < file_content.size(); ++line_index) {
		...
		if (line[0] == '.') {
			...
			if (pseudo_command == ".ORIG") {
				...
				// TO BE DONE
				file_address[line_index] = line_address;
				std::string fill_content;
				line_stringstream >> fill_content;
				line_address += 1;
			}
			else if (pseudo_command == ".BLKW") {
				// TO BE DONE
				file_address[line_index] = line_address;
				std::string blocksize_str;
				line_stringstream >> blocksize_str;
				auto blocksize = RecognizeNumberValue(blocksize_str);
				if (blocksize == std::numeric_limits<int>::max()) {
					// @ Error Invalid Number input @ FILL
					return -4;
				}
				if (blocksize > 65535 || blocksize < -65536) {
					// @ Error Too large or too small value  @ FILL
					return -5;
				}
				line_address += blocksize;
			}
			else {
	...
	for (int line_index = 0; line_index < file_content.size(); ++line_index) {
		...
		if (file_tag[line_index] == lOperation) {
			...
			while (1)
			{
				if (parameter_str.find(',') == std::string::npos)
					break;
				else
					parameter_str = parameter_str.replace(parameter_str.find(','), 1, " ");
			}
